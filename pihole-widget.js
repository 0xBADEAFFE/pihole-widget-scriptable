...
if (config.runsInWidget) {
  const widgetParams =
    args.widgetParameter != null ? JSON.parse(args.widgetParameter) : null;
  if (widgetParams == null) {
    throw new Error("Please long press the widget and add the parameters.");
  } else if (
    !widgetParams.hasOwnProperty("url") &&
    !widgetParams.hasOwnProperty("apikey")
  ) {
...
async function createWidget() {
  ...
  let state =
    piholeStats != null ? (dns.blocking == "enabled" ? true : false) : null;
  let icn = null;
  ...
  if (state == true && wSize != "small") {
    ...
    addUpdateItem(
      topStack,
      version.version.core.local.version == version.version.core.remote.version
        ? true
        : false,
      "Pi" + (wSize == "small" ? "" : "-hole")
    );
    ...
    addUpdateItem(
      topStack,
      version.version.web.local.version == version.version.web.remote.version
        ? true
        : false,
      "WebUI"
    );
    ...
    addUpdateItem(
      topStack,
      version.version.ftl.local.version == version.version.ftl.remote.version
        ? true
        : false,
      "FTL"
    );
  ...
  addItem(
    w,
    "Percent Blocked",
    piholeStats.queries.percent_blocked.toFixed(1) + "%"
  );
  ...
  addItem(
    ...
    `${piholeStats.gravity.domains_being_blocked}`
  );
